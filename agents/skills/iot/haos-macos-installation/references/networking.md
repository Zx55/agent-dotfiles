...
Use this only to verify or repair the current HAOS-on-UTM network path. The normal routing setup is owned by the HA host orchestrator under `bootstrap/ha_host/tools/orchestrator/`, not by scripts inside this skill.
