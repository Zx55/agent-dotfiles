...
```text
bootstrap/ha_host/tools/orchestrator/
```

If the installed copy needs to be repaired, use that tool's own README and installer from `bootstrap/ha_host/tools/orchestrator/`. Keep that operation separate from HAOS network configuration.
