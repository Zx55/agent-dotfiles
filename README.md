...
`shared/skills/` contains the canonical skill sources. Profile directories under `*/agent/skills/` contain only symlinks to the shared skill directories, so profile skill selection is managed by filesystem links instead of bootstrap script lists. Profiles publish their selected skills to `~/.agents/skills/<category>/<skill>` so Codex, Cursor, Copilot, and other Agent Skills-aware harnesses use one runtime projection on machines that serve multiple roles.
...
Both profiles use symlinks for agent-specific instructions, profile-managed Codex files, skills, and dotfiles. Existing config and dotfile targets are backed up under `~/.dotfiles-backup/<timestamp>/` before replacement. Profile skills are published under `~/.agents/skills` so one machine can be both the master workstation and HA host without splitting shared Agent Skills by harness. Occupied shared skill paths cause `links` to stop instead of replacing another skill provider's content.
