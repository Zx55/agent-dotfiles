...
- If the existing workspace config is healthy, pass `--skip-init` unless the user explicitly wants first-run setup or provider config refresh.
- Do not run `--overwrite-init` just because a version update happened.
...
- init/config changes: provider options, workspace files, prompts, required overwrite
...
```bash
rg -n "dayu-cli|dayu-render|upload_filing|upload_filings_from|upload_material|download|interactive|prompt|runs|host|sessions|write" \
  ~/Documents/codex-workspace/agent-dotfiles/skills/finance/dayu \
  ~/Documents/codex-workspace/agent-dotfiles/skills/finance/dayu-installation
```
...
Some releases add or change provider setup. If the release notes say a provider requires rerunning init, make the overwrite explicit and protect existing config:

```bash
dayu-cli init --base ~/.dayu/workspace --overwrite
```

Only run this when the user wants the provider/config refresh, because it replaces existing workspace config. If the provider is OpenAI-compatible and init does not produce the desired config, continue with [openai_compatible_provider.md](openai_compatible_provider.md).

Current version-specific note:

- Dayu `v0.1.2` says MiMo Plan overseas support requires rerunning `dayu-cli init --overwrite`.

## Offline packages

Dayu `v0.1.2+` publishes offline packages for macOS ARM64, Linux x64, and Windows x64. Treat offline packages as a fallback when network wheel installation is not viable. The preferred local agent setup remains `uv tool install` from the release wheel, because it keeps `dayu-cli`, `dayu-render`, and `dayu-wechat` in the normal `uv` tool inventory.
