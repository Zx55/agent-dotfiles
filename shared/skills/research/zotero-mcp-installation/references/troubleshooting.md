...
- Restart Zotero.
- Use the same token in the MCP server env as `ZOTERO_ADD_LOCAL_FILE_TOKEN`.
...
- If `mcp-launcher` is installed, agent MCP config should use `mcp-launcher` as the command and pass `~/.local/bin/zotero-mcp-wrapper` after `--`.
- If `mcp-launcher` is unavailable, direct `zotero-mcp-wrapper` config is an acceptable machine-local fallback.
- `mcp-launcher` exists when using the preferred portable config.
- `~/.local/bin/zotero-mcp` exists and is executable.
- `~/.local/bin/zotero-mcp-wrapper` exists and is executable.
- Agent MCP config uses either `mcp-launcher` as the command or the direct wrapper fallback.
