...
usage() {
  cat <<'EOF'
Usage: ha-host/bootstrap/bootstrap.sh install [options]
...
log() {
  printf '[ha-host:install] %s\n' "$*"
}

die() {
  printf '[ha-host:install] error: %s\n' "$*" >&2
  exit 1
}
...
configure_power_policy() {
  if [[ "$DRY_RUN" -eq 1 ]]; then
    log "Would set AC power policy: sleep=$AC_SLEEP displaysleep=$AC_DISPLAY_SLEEP disksleep=$AC_DISK_SLEEP"
    return 0
  fi

  command -v pmset >/dev/null 2>&1 || die "pmset is required to configure macOS power policy"

  log "Setting AC power policy: sleep=$AC_SLEEP displaysleep=$AC_DISPLAY_SLEEP disksleep=$AC_DISK_SLEEP"
  sudo pmset -c sleep "$AC_SLEEP" displaysleep "$AC_DISPLAY_SLEEP" disksleep "$AC_DISK_SLEEP"
}
